...
public class UpdatePriceServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        DistanceV2 d = distancedao.getDistanceById(id);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
                throws ServletException, IOException {
        //processRequest(request, response);
        String distanceID = request.getParameter("distance_ID");
        float distance = Float.parseFloat(request.getParameter("distance"));
        float price = Float.parseFloat(request.getParameter("price"));

        DistanceDAO distancedao = new DistanceDAO();
        DistanceV2 d = new DistanceV2(distanceID, distance, price);
        distancedao.update(d);
        response.sendRedirect("ListDistanceServlet");
    }
