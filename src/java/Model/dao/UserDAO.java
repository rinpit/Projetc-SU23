...
public class UserDAO {
    ...
    public List<User> readEmployee() throws ClassNotFoundException {
        ...
        try {
            ResultSet resultSet;
            String statement = "SELECT * FROM Users WHERE Role='employee'";
            PreparedStatement preparedStatement = connection.prepareStatement(statement);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
    ...
    public List<String> readEmails() throws ClassNotFoundException {
        ...
        try {
            ResultSet resultSet;
            String statement = "SELECT Users.Email FROM Users";
            PreparedStatement preparedStatement = connection.prepareStatement(statement);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
...
//    }
    ...
    public ArrayList<User> getCustomer() throws SQLException {
        ...
        rs = ps.executeQuery();
        while (rs.next()) {
            customers.add(new User(rs.getString(2), rs.getString(3), rs.getDate(4),
    ...
    public ArrayList<User> getCustomerByPage(String page) throws SQLException {
        ...
        rs = ps.executeQuery();
        while (rs.next()) {
            custs.add(new User(rs.getString(2), rs.getString(3), rs.getDate(4),
                    rs.getString(5), rs.getString(1), rs.getString(6),
                    rs.getString(7), rs.getString(8)));
        }
        return custs;
